...
class ImagePredictor {
    ...
    static func createImageClassifier() -> VNCoreMLModel {
        ...
        guard let imageClassifierModel = try? resnet18().model else {
            fatalError("App failed to load the model file.")
        }
...
//        }
        ...
        // Extract raw confidences as Float array
        let rawConfidences = observations.map { $0.confidence }

        // Compute softmax over raw confidences
        let softmaxConfidences = softmax(rawConfidences)

        // Map observations with softmaxed confidences
        predictions = zip(observations, softmaxConfidences).map { observation, softmaxConfidence in
            Prediction(
                classification: observation.identifier,
                confidencePercentage: String(format: "%.2f", softmaxConfidence * 100)
            )
        }
    }
    
    func softmax(_ logits: [VNConfidence]) -> [Float] {
